...
class IngredientAmountTest < ActiveSupport::TestCase
  setup do
    ...
    @i1_3 = IngredientAmount.new(ingredient_id: 1, quantity: 1.5, unit: "tablespoons")
    @i2 = IngredientAmount.new(ingredient_id: 2, quantity: 1.5)
  end

  test "sum_by_id aggregates multiple different IngredientAmount" do
    sum = IngredientAmount.sum_by_id([@i1_1, @i1_2, @i2])
    assert sum[0].serialize == IngredientAmount.mk(1, 3.9).serialize
    assert sum[1].serialize == @i2.serialize
  end

  test "sum_by_id keeps same id separate if different units" do
    sum = IngredientAmount.sum_by_id([@i1_1, @i1_3, @i1_2, @i1_3])
    assert sum[0].serialize == IngredientAmount.mk(1, 3.9).serialize
    assert sum[1].serialize == IngredientAmount.mk(1, 3.0, "tablespoons").serialize
  end
