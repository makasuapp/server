...
class OpDayManager
  ...
  def self.update_day_for(date, kitchen)
    ...
    self.create_day(predicted_orders, op_day, ingredient_qtys, made_qtys)
  end
  ...
  # aggregate the amount of ingredients/prep for the day
  ...
  sig {params(
    ...
    op_day: OpDay, 
    ingredient_had_qtys: T::Hash[Integer, T::Array[IngredientAmount]], 
    prep_made_qtys: T::Hash[Integer, T::Array[StepAmount]]
    ).void} 
  def self.create_day(predicted_orders, op_day, ingredient_had_qtys, prep_made_qtys)
    day_preps_map = {}
    day_ingredients_map = {}
    ...
    predicted_orders.includes({recipe: {recipe_steps: :inputs}}).each do |po|
      ...
      subrecipe_step_amounts, ingredient_amounts = recipe.component_amounts(for_date)
      ...
      ingredient_amounts.each do |i_amount|
        ...
        ingredient_qty = i_amount.quantity * recipe_servings
        #TODO(timezone)
        ...
        if matches_idx.nil?
          ...
          additional_qty = UnitConverter.convert(ingredient_qty, i_amount.unit,
    ...
    DayIngredient.import! day_ingredients_map.values.flatten
