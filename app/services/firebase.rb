...
class Firebase
  ...
  def send_data(topic_name, message_type, json_str)
    @fcm.send_to_topic(topic_name, data: {type: message_type, content: json_str})
  end
  ...
  def send_notification_with_data(topic_name, body, message_type, json_str, title = nil)
    @fcm.send_to_topic(topic_name, notification: {title: title, body: body}, data: {type: message_type, content: json_str})
  end
end